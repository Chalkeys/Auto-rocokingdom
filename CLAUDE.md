...
```bash
uv sync
uv run python app/gui.py      # AutoRoco（会自动请求管理员权限）
uv run python app/flower.py   # 刷花助手
uv run python launcher.py     # 统一启动器
```
...
**Remote setup:**
- `github` — GitHub remote (`chalkeys/auto-rocokingdom`), for major releases only
- No default `origin`; all daily work stays local (Dropbox handles backup via sync)

**Daily work:** `git commit` only — do NOT push automatically.

**Major release (new minor/major version):**
```bash
git push github main --tags
```

**IMPORTANT for Claude:** Never run `git push` automatically. Only commit locally unless the user explicitly requests a push to GitHub.
